...
- **Passar llista per classe i dia**: tries una classe i una data, veus tots els alumnes en una graella de caselles (matí / tarda / retard, cadascuna amb la seva pròpia casella de "justificat"), i guardes tot el dia d'un cop. Una falta de matí i un retard de matí són mutuament excloents i comparteixen la mateixa casella de "justificat", ja que només un dels dos pot haver passat de veritat.
- **Resum d'absències en temps real**: percentatge d'assistència automàtic per alumne (ponderat — faltar un matí no compta igual que faltar una tarda, seguint l'horari real de l'escola), desglossat entre falta justificada/no justificada, un desglossament també per als retards (justificats/no justificats), un nivell d'alerta progressiu per colors, un desglossament per dia de la setmana, un cercador i columnes ordenables.
- **Control d'accés**: restringit a una llista explícita d'emails de professorat autoritzat, comprovada al servidor a cada petició — no només un enllaç difícil d'endevinar. Els fulls d'administració (configuració, llistat d'alumnes, llista de professorat, mapa de promoció) estan protegits a nivell de la pròpia Sheet perquè només la persona propietària del fitxer els pugui editar, encara que la resta de professorat necessiti accés d'Editor perquè funcioni l'aplicació.
...
5. Desa el projecte.
6. Al desplegable de funcions de dalt, selecciona **`setup`** i pulsa **Executar**. La primera vegada et demanarà autoritzar l'script — accepta-ho. Això crea els fulls `Config`, `Students`, `Teachers`, `Records` i `Promotion` amb dades d'exemple i valors per defecte raonables, i protegeix tot excepte `Records` perquè només la persona propietària de la Sheet ho pugui editar.
